...
→구독한 시점부터 데이터를 새로 받아오는 Subject

subscribe 하면 Observer 여러개

### BehaviorSubject

-> 구독하면 subscribe()한 시점에서의 마지막데이터까지 받아온다.

★생산하는 곳이 여러곳이 있을 경우 이걸 사용(ex. RxBus)

### ReplaySubject

subscribe()하면 지금까지 발행된 데이터를 전부 받아옴

### AsyncSubject

Complete()가 되면 가장 마지막 데이터를 받게 됨